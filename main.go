...
var (
	...
	callgraphAlgo = flag.String("algo", string(CallGraphTypeStatic), fmt.Sprintf("The algorithm used to construct the call graph. Possible values include: %q, %q, %q",
...
func outputDot(fname string, outputFormat string) {
	...
	writeErr := ioutil.WriteFile(fmt.Sprintf("%s.gv", fname), output, 0o755)
	if writeErr != nil {
		log.Fatalf("%v\n", writeErr)
	}
